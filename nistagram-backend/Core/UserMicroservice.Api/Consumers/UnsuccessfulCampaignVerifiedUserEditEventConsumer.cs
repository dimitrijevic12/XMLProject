...
{
    public class UnsuccessfulCampaignVerifiedUserEditEventConsumer : IConsumeAsync<UnsuccessfulCampaignVerifiedUserEditEvent>
    {
        private readonly UserService userService;
        private readonly IUserRepository _userRepository;
        private readonly IBus _bus;

        public UnsuccessfulCampaignVerifiedUserEditEventConsumer(UserService userService, IBus bus, IUserRepository userRepository)
        {
        ...
        public async Task ConsumeAsync(UnsuccessfulCampaignVerifiedUserEditEvent message, CancellationToken cancellationToken = default)
        {
            await userService.RejectVerifiedUserEditAsync(Convert(message), "Unsuccessful user edit error!");
        }

        private VerifiedUser Convert(UnsuccessfulCampaignVerifiedUserEditEvent user)
        {
            return VerifiedUser.Create(new Guid(user.Id), Username.Create(user.Username).Value, EmailAddress.Create(user.EmailAddress).Value, FirstName.Create(user.FirstName).Value,
            ...
            CreateUsers(user.MyCloseFriends), CreateUsers(user.CloseFriendTo), user.IsBanned, Category.Create(user.Category).Value).Value;
        }
