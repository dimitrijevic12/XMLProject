...
{
    public class UnsuccessfulCampaignAgentEditEventConsumer : IConsumeAsync<UnsuccessfulCampaignAgentEditEvent>
    {
        private readonly UserService userService;
        private readonly IUserRepository _userRepository;
        private readonly IBus _bus;

        public UnsuccessfulCampaignAgentEditEventConsumer(UserService userService, IBus bus, IUserRepository userRepository)
        {
        ...
        public async Task ConsumeAsync(UnsuccessfulCampaignAgentEditEvent message, CancellationToken cancellationToken = default)
        {
            await userService.RejectAgentEditAsync(Convert(message), "Unsuccessful user edit error!");
        }

        private Agent Convert(UnsuccessfulCampaignAgentEditEvent user)
        {
            return Agent.Create(new Guid(user.Id), Username.Create(user.Username).Value, EmailAddress.Create(user.EmailAddress).Value, FirstName.Create(user.FirstName).Value,
